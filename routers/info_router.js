...
const system_info_1 = require("../common/system_info");
// 获取守护进程系统基本信息
router_1.routerApp.on("info/overview", async (ctx) => {
    let total = 0;
    let running = 0;
    system_instance_1.default.instances.forEach((v) => {
        total++;
        if (v.status() == instance_1.default.STATUS_RUNNING)
            running++;
    });
    const info = {
        process: {
        ...
        system: system_info_1.systemInfo()
    };
